...
/* Parse SAML configuration string and return config file path */
...
static void saml_vhost_init(void **_vctx, void *pool, void *additional)
{
	...
	if (vctx->server == NULL) {
		fprintf(stderr,
			"Error initializing Lasso SAML server object. Check configuration. It's almost always the metadata.\n");
		exit(1);
	}
	...
	if (idp_list == NULL || idp_list->data == NULL) {
		fprintf(stderr,
			"Error: No identity provider found in configuration. Check metadata.\n");
	...
	*_vctx = (void *)vctx;
	 ...
	 * to serve upon request. Drop a copy in /tmp */
	...
	if (fptrSrc == NULL) {
		fprintf(stderr, "Cannot open SP metadata file: %s\n", config->spmeta);
		return;
	}
	fptrDest = fopen(tmpSpMetaFile, "w");
	if (fptrDest == NULL) {
		fprintf(stderr, "Cannot write to /tmp/spmeta.xml\n");
		fclose(fptrSrc);
		return;
	}
			  ...
			  const common_auth_init_st * info)
{
	...
	if (ret != 0) {
		fprintf(stderr, "Failed building authn request\n");
	}
...
static int saml_urldecode(char *data)
{
	char *ip;
	char *op;
	int c1, c2;
				 ...
				 const char *url)
{
	...
	if (scd->Recipient) {
		if (strcmp(scd->Recipient, url)) {
			fprintf(stderr,
				"Wrong Recipient in SubjectConfirmationData. "
			  ...
			  unsigned pass_len)
{
	...
	if (response->parent.Destination) {
		if (strcmp(response->parent.Destination, ctx->vctx->config->acs_url)) {
			fprintf(stderr,
