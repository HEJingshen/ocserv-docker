...
class StaticConfigTest(unittest.TestCase):
    ...
    def test_auth_docker_build_context_and_ci_are_validated(self):
        ...
        self.assertIn("curl -fsSL -o src/ocserv-", self.workflow)
        self.assertIn('echo "BUILD_DATE=$(date -u +\'%Y-%m-%dT%H:%M:%SZ\')" >> "$GITHUB_OUTPUT"', self.workflow)
        self.assertIn("runs-on: ${{ matrix.platform.runner }}", self.workflow)
        self.assertIn("runner: ubuntu-24.04-arm", self.workflow)
        self.assertIn("matrix.image.type", self.workflow)
        self.assertIn("matrix.platform.arch", self.workflow)
        self.assertIn("name: Build ${{ matrix.image.type }} (${{ matrix.platform.arch }})", self.workflow)
        self.assertIn("name_var: OCSERV_AUTH_IMAGE_NAME", self.workflow)
